...
export function useAIConfig() {
    ...
    useEffect(() => {
        try {
            ...
            window.dispatchEvent(new CustomEvent('aiconfig-changed', { detail: aiConfig }));
        } catch (e) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('aiconfig-changed', handleConfigChange as EventListener);
        return () => {
            window.removeEventListener('aiconfig-changed', handleConfigChange as EventListener);
        };
    }, []);

    // Validate config values before updating
    const setAIConfig = useCallback((newConfig: Partial<AIConfig> | ((prev: AIConfig) => AIConfig)) => {
        setAIConfigState(prev => {
            const nextConfig = typeof newConfig === 'function' ? newConfig(prev) : { ...prev, ...newConfig };
            
            // Validate numeric fields
            if (typeof nextConfig.temperature === 'number') {
                nextConfig.temperature = Math.max(0, Math.min(1, nextConfig.temperature));
            }
            ...
            if (typeof nextConfig.maxRetries === 'number') {
                nextConfig.maxRetries = Math.max(0, Math.floor(nextConfig.maxRetries));
            }
