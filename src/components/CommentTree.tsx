...
const CommentTree: React.FC<CommentTreeProps> = ({ 
  ...
  const findAndRemoveComment = (comments: CommentType[], id: string): [CommentType | null, CommentType[]] => {
  ...
  const isDescendant = (parentComment: CommentType, childId: string): boolean => {
  ...
  const handlePopUp = (commentId: string) => {
    ...
    const [commentToPopUp, remainingComments] = findAndRemoveComment(allComments, commentId);
